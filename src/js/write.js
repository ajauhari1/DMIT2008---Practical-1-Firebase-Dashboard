...
document.forms["movieForm"].addEventListener("submit", onAddMovie); 
   ...
   function onImageSelected(e) {
    ...
    async function uploadNewMovie() {
        ...
        // ref to RTD store ...using movies/
        const dataRef =  databaseRef( db, 'movies') // ref to RTD store
        // firebase unique key
        // push used to return a ref to the area with they key but no data is written 
        const itemRef = await push(dataRef)
        // paths to the data to write for our image 
        ...
        .then(()=>{
            alert("Add successful...Main page to see results")
        })
    }
